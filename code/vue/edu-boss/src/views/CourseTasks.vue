  ...
  <section class="course-tasks">
    ...
    >
      ...
      <div class="inner" slot-scope="{ data, node }">
        ...
        <span class="actions">
          ...
          <!-- 添加课时 -->
          <el-button v-if="node.level == 1" size="small" @click.stop="handleShowAddLesson(data.course_id,data.id)">添加课时</el-button>
          <el-button v-if="node.level == 2" size="small" @click.stop="handleEditLesson(data)">修改课时</el-button>
    ...
    <el-dialog class="add-dialog" title="课时信息" :visible.sync="showAddLesson">
      <el-form
        ...
        :model="addLessonForm"
      >
        <el-form-item label="课时id" prop="id" >
          <el-input v-model="addLessonForm.id" disabled ></el-input>
        </el-form-item>
        <el-form-item label="课时主题" prop="theme">
          <el-input v-model="addLessonForm.theme"></el-input>
        </el-form-item>
        <el-form-item label="课时时长" prop="duration">
          <el-input v-model="addLessonForm.duration"></el-input>
        </el-form-item>
        <el-form-item label="课时收费" prop="is_free">
          <el-input type="textarea" v-model="addLessonForm.is_free"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    //方法10: 显示添加课时表单,回显课时信息
    handleShowAddLesson(cid,sid) {
      this.addLessonForm = {
        course_id: cid,
        section_id: sid
      };
