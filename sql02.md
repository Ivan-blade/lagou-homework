...
# 3) 查询大于5000的员工，来至于哪些部门，输出部门的名字
 SELECT DISTINCT d.name FROM employee e,dept d WHERE e.`salary` > 5000; 
# 4) 查询开发部与财务部所有的员工信息，分别使用子查询和表连接实现
	-- 查询表 员工表，部门表
	-- 查询目标 员工表中的员工信息
	-- 过滤条件 员工部门id 对应 部门表中的部门id 对应的部门是开发部和财务部
SELECT 
 *
FROM employee e LEFT JOIN dept d
ON e.`dept_id` = d.`id`
WHERE d.id IN (SELECT id FROM dept WHERE dept.`name` = '开发部' OR dept.`name` = '财务部');
# 5) 查询2011年以后入职的员工信息和部门信息，分别使用子查询和表连接实现
SELECT 
* 
FROM employee e LEFT JOIN dept d
ON  e.`dept_id` = d.`id`
WHERE e.`join_date` IN (SELECT join_date FROM employee WHERE join_date > '2010-12-31');
```
